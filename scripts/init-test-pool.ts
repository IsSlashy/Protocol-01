/**
 * Initialize Test Shielded Pool on Devnet
 *
 * Creates a new pool with a unique token mint for clean testing.
 * Run with: npx ts-node scripts/init-test-pool.ts
 */
...
// Test token mint - using a fixed keypair for reproducibility
// This is NOT native SOL, just a unique identifier for a test pool
const TEST_TOKEN_MINT = new PublicKey('Testy1111111111111111111111111111111111111');

async function main() {
  console.log('===========================================');
  console.log('Initialize TEST Shielded Pool');
  console.log('===========================================\n');
  ...
  console.log('Test Token Mint:', TEST_TOKEN_MINT.toBase58());
  ...
  const [poolPDA, poolBump] = PublicKey.findProgramAddressSync(
    [SHIELDED_POOL_SEED, TEST_TOKEN_MINT.toBytes()],
  ...
  if (poolInfo) {
    ...
    const merkleInfo = await connection.getAccountInfo(merkleTreePDA);
    if (merkleInfo) {
      // Parse leaf_count (at offset 8 after discriminator)
      const leafCount = merkleInfo.data.readUInt32LE(8);
      console.log('Current leaf count:', leafCount);
    }
    return;
  }
  ...
  // token_mint - 32 bytes
  const tokenMintBytes = TEST_TOKEN_MINT.toBytes();
  ...
  try {
    ...
    console.log('\n===========================================');
    console.log('IMPORTANT: Update your app to use this pool');
    console.log('===========================================');
    console.log('Token Mint:', TEST_TOKEN_MINT.toBase58());
