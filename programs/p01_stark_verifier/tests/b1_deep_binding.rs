...
///
/// [B7 2026-08-04] ALL SEVEN moved with the coset LDE (`h = 7`): shifting the
/// committed evaluation domain off the trace domain changes every committed
/// vector and therefore every proof, at unchanged lengths. Pinned HERE FIRST
/// (commit e900b78c) from the Rust prover; the WASM blob was then rebuilt and
/// reproduced all seven byte for byte (51a947e3…, 229,640 B), and the three
/// JS/JSON twins — `wireFormat.test.ts`, `scripts/prover-behaviour.mjs`,
/// `deployed-verifier.json` — were re-pinned in the SAME commit as that reship,
/// per the rule the [BIND-C2C4] block below records. The superseded pre-coset
