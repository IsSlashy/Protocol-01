...
/// Seed for unshield VK data PDA
pub const VK_DATA_SEED: &[u8] = b"trustless_vk_data";
...
/// Initialize VK data account for the unshield circuit.
...
pub struct InitVkData<'info> {
    ...
    #[account(
        mut,
        seeds = [VK_DATA_SEED, pool.key().as_ref()],
        bump
    )]
...
/// Write chunk of VK data for the unshield circuit.
#[derive(Accounts)]
