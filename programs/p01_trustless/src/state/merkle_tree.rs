...
/// Merkle tree state for the trustless pool.
/// Stores filled subtrees for efficient insertion.
/// Root is computed off-chain (Poseidon syscall not yet on mainnet).
#[account]
#[derive(Default)]
pub struct MerkleTreeState {
    ...
    /// Filled subtrees (rightmost filled node at each level)
...
impl MerkleTreeState {
    ...
    /// Poseidon syscall is not yet enabled on devnet/mainnet,
    /// so the client computes the new root off-chain.
    ...
    /// Matches the circomlibjs Poseidon implementation used in the ZK circuit.
