...
/// G1 point size (uncompressed): 64 bytes
const G1_SIZE: usize = 64;
/// G2 point size (uncompressed): 128 bytes
...
impl Groth16Verifier {
    ...
    /// Verify an unshield proof for the trustless denominated pool.
    ///
    ...
    /// Verify a confidential balance proof for zkSPL.
    ///
    ...
    fn parse_vk(vk_data: &[u8]) -> Result<VerificationKeyData> {
        ...
        if vk_data.len() < min_size {
            return Err(TrustlessError::InvalidVerificationKey.into());
        }
    ...
    fn compute_ic_sum(public_inputs: &[[u8; 32]], ic: &[[u8; G1_SIZE]]) -> Result<[u8; G1_SIZE]> {
        if public_inputs.len() + 1 != ic.len() {
            return Err(TrustlessError::InvalidPublicInputs.into());
        }
    ...
    fn g1_add(p1: &[u8; G1_SIZE], p2: &[u8; G1_SIZE]) -> Result<[u8; G1_SIZE]> {
        ...
        #[cfg(target_os = "solana")]
        {
            let result_vec = alt_bn128_addition(&input)
                .map_err(|_| TrustlessError::InvalidProof)?;
    ...
    /// Negate G1 point (negate y coordinate in the BN254 Fq field).
