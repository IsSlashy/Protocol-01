#!/usr/bin/env node
/**
 * Generator for the FRESH-ROOT fixture — see README.md in this directory.
 *
 ...
 * spend that proves against the NEWEST Merkle root the tree had at spend time,
 * so that `--max-root-age` can assert P12 PASS with measure 0 on frozen bytes.
 *
 * ⛔ IT IS THE TWIN OF `../v4-stale-root/generate.mjs`, AND THAT IS THE CONTROL.
 ...
 * swap: this fixture's green cannot come from anything the stale one does not
 * also have, and a P12 that stopped reading the root would turn the other red.
 ...
 *
 * WHY A POSITIVE HALF AT ALL. `v4-stale-root` alone proves only that the probe
 * can say FAIL, and a probe that hard-failed every spend would satisfy it while
 * measuring nothing. This fixture is what makes a future green meaningful: it
 * is the same world with a fresh root, and P12 must say so. It is the same
 * argument `v4-synthetic` makes for P1/P2/P4 next door.
 *
...
// The insertion whose `new_root` the spend names, chosen so the root is age 0
// (LEAVES - (NAMED_LEAF + 1)). v4-stale-root chooses age 4. The generated text
...
const calls = [
  ...
  // address belonging to nobody. MEASURED on this fixture's twin before P12
  // existed — the old reading walked `14YBZAim…` and the replay hard-stopped.
...
const manifest = {
  ...
  note:
    ...
    'spend that proves against the newest root the tree had, so P12 measures age 0 and passes. ' +
    'It is the positive half of the root-age control ' +
    'pair — without it, a P12 that failed every spend would satisfy verify/fixtures/v4-stale-root ' +
    'and measure nothing. The two fixtures are one world and differ in one field, the 32 bytes of ' +
    'merkle_root. It is NOT evidence that any real spend names a fresh root: the client that ' +
    'sends the stored path is what decides that, and it was MEASURED naming a 4-insertion-old ' +
    'root on devnet 2026-09-15.',
  spend: spendSig,
  ...
  // Identical to the stale twin except P12. Everything else is pinned here for
  // the same reasons, and pinning them twice is what makes the pair a control:
  // the two runs differ in exactly one verdict and one measure.
  //
  // P1/P2/P4 PASS: no commitment is published, nothing correlates.
  ...
  // P12 PASS, measure 0: the named root is the tree's own state at spend time,
  //   so it dates no deposit. THIS IS THE ONE LINE THAT DIFFERS FROM THE TWIN.
  ...
  measure: { P6: 0, P10: 0, P12: 0, P13: 1 },
