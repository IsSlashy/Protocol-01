...
// scripts/sync-program-ids.mjs. Run `pnpm sync-program-ids` after editing
// Anchor.toml to regenerate it. Do not hand-edit between the sentinels.
//
// That path said scripts/sync-program-ids.TS until 2026-08-02 and no such file
// had ever existed, so `pnpm sync-program-ids` and `pnpm check-program-ids` both
// died on a missing module before checking anything while this comment, and two
// files in packages/stark-prover, cited them as the thing that caught id drift.
// The generator is dependency-free `.mjs` on purpose: a gate that needs `pnpm
// install` to have happened is a gate that silently does not run.
//
// `pnpm check-program-ids` is RED as of 2026-08-02 and correctly so. It reports
// that Anchor.toml names a CLOSED devnet account for zk_shielded and accounts
// that do not exist for specter and p01_relayer, all three measured off devnet,
// and that ~20 hardcoded ids across other packages address programs at an
// address their own declare_id! forbids. Those are outside the pass that wrote
// this comment; the p01_stark_verifier divergence it was written for is fixed.
