/**
 * Goldilocks-Poseidon (pure TS port — privacy-sdk).
 *
 * Binary-identical twin of `apps/mobile/services/zk/goldilocks-poseidon.ts`
 * and the Rust reference in `stark/src/poseidon/{mod,constants}.rs`. The SDK
 * must reproduce the exact same hashes byte-for-byte so shielded notes created
 * via the SDK are spendable by the same user's mobile/extension clients and
 * verify on-chain through p01_stark_verifier (circuits 5 + 6).
 *
 * Field:    Goldilocks  p = 2^64 - 2^32 + 1
 * S-box:    x^7 (full S-box on every round — no partial rounds)
 * Rounds:   30
 * MDS(t=3): circulant [[3,1,1],[1,3,1],[1,1,3]]
 * MDS(t=5): circulant [[5,1,1,1,1], …]
 *
 * WARNING: do not reorder, truncate, or reformat the constants. Any drift
 * (even a single bit) = immediate on-chain verification failure. Parity is
 * checked in `packages/privacy-sdk/tests/goldilocks-poseidon.parity.test.ts`
 * (P1.5).
 */
...
type Row3 = readonly [bigint, bigint, bigint];
type Row5 = readonly [bigint, bigint, bigint, bigint, bigint];

export const MDS_MATRIX_T3: readonly [Row3, Row3, Row3] = [
  [3n, 1n, 1n],
  [1n, 3n, 1n],
  [1n, 1n, 3n],
];

export const MDS_MATRIX_T5: readonly [Row5, Row5, Row5, Row5, Row5] = [
  [5n, 1n, 1n, 1n, 1n],
  [1n, 5n, 1n, 1n, 1n],
  [1n, 1n, 5n, 1n, 1n],
  [1n, 1n, 1n, 5n, 1n],
...
function mdsMultiplyT5(state: bigint[]): void {
  ...
  for (let i = 0; i < 5; i++) {
    let acc = 0n;
    const row = MDS_MATRIX_T5[i as 0 | 1 | 2 | 3 | 4];
    for (let j = 0; j < 5; j++) {
      acc = gadd(acc, gmul(row[j as 0 | 1 | 2 | 3 | 4], state[j]!));
    }
    result[i] = acc;
  }
