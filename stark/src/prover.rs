//! Winterfell reference prover and verifier for the AIR definitions.
//!
//! `prove_generic` / `verify_generic` are the test oracle every `air/*.rs`
//! module checks its constraints against. They are not what ships: the
//! clients run the compact prover in `crate::compact`, through `mod wasm_api`.
