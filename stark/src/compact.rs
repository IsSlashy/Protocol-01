...
///
/// Re-measured after the round-2 review fixes against `.so` sha256 `e13073c6…`,
/// still 638,248 B: every phase-1 CU and every proof size below reproduced
/// BIT-IDENTICALLY. The `.so` hash moved while the size did not because the
/// round-2 edits to `verify.rs` were comments and `#[cfg(test)]` code, which change
/// the crate fingerprint rustc mixes into mangled symbol names. Identical CU on all
/// seven circuits is strong evidence of functional equivalence — it is evidence,
/// not a proof of bit-identical semantics, and is labelled as such.
///
/// ```text
