//! AIR-AWARE RECOVERY ON C2: the carry column that used to hand over `owner_mint`.
//!
...
//! exact `owner_mint`.
//!
//! WHAT C2 LEAKED, AND WHY IT WAS WORSE THAN C1
//! ───────────────────────────────────────────
//! `docs/AUDIT-2026-09-03.md` recorded it on C4 and C2 has the identical
//! column: col 3 is a TWO-SEGMENT STEP -- zero on rows 0..63, `owner_mint` on
//! rows 64..127 -- so the whole column was ONE unknown once the AIR's linear
//! structure was written down, and the wire publishes `4 * 27 + 2 = 110`
//! evaluations of it. One field inversion returned `owner_mint =
...
//! ─────────────────────
//! Since 2026-09-11 the trace is 512 rows and rows 128..511 of col 3 are fresh
//! CSPRNG draws: 1 + 384 unknowns against 110 openings, and the linear system
//! is under-determined by 275. That is what this file measures, with the
...
//! `stark/src/compact/zk_hiding.rs` (`Circ::C2`) and counted in
//! `stark/tests/full_wire_ledger.rs`.
//!
//! Run: `cargo test -p p01-stark --release --test air_aware_recovery_c2 -- --nocapture`

use p01_stark::compact::generate_balance_compact_proof;
use p01_stark::air::balance_proof as c2;
...
// C2 geometry, READ OFF THE AIR. Verifier twin: `CONFIG_BALANCE_PROOF`.
const TRACE_LEN: usize = c2::TRACE_LENGTH;
const TRACE_WIDTH: usize = c2::TRACE_WIDTH;
const CW: usize = c2::CONSTRAINED_TRACE_WIDTH;
const FIRST_FREE_ROW: usize = c2::FIRST_FREE_ROW;
...
fn self_check_field() {
    assert_eq!(TRACE_LEN, 512, "this harness is written for the 512-row C2");
...
///
/// Rows 0..63 are zero (the carry is captured at row 63), rows 64..127 are all
/// equal (carry continuity, gated by `active` up to row 126 -> 127). Rows
/// 128..511 are fresh randomness and contribute NO equality.
fn carry_structure_masked() -> Vec<Vec<u64>> {
    let mut eqs = Vec::new();
    for r in 0..(2 * c2::HASH_CYCLE_LEN) {
        eq_zero(&mut eqs, r);
    }
    for r in (2 * c2::HASH_CYCLE_LEN + 1)..FIRST_FREE_ROW {
        eq_cells(&mut eqs, r, 2 * c2::HASH_CYCLE_LEN);
    }
    eqs
}

/// The PRE-MASK model: the column was `owner_mint` on every row past 63, and
/// the trace ended at row 127. Written over today's 512 rows it says every cell
...
const SALT: u64 = 0x0BAD_C0FF_EE00_1234;
...
fn the_pre_mask_carry_column_hands_over_owner_mint() {
    ...
    // the carry column set to `owner_mint`. This is what C2 published before
    // 2026-09-11 (with the trace simply ending at row 127).
    let om = owner_mint();
    let mut mask = test_mask(0xC2_5EED_0003);
    for row in FIRST_FREE_ROW..TRACE_LEN {
        mask[(row - FIRST_FREE_ROW) * CW + CARRY_COL] = om;
    }
    let proof = generate_balance_compact_proof(SPENDING_KEY, BALANCE, SALT, TOKEN_MINT, &mask);
...
fn the_mask_closes_the_carry_column() {
    ...
    let proof = generate_balance_compact_proof(SPENDING_KEY, BALANCE, SALT, TOKEN_MINT, &test_mask(0xC2_5EED_0003));
    let op = parse_generic(&proof.proof_bytes);
    let nodes = published_nodes(&op, CARRY_COL);
...
fn every_constrained_column_is_under_determined() {
    self_check_field();
    let proof = generate_balance_compact_proof(1, 2, 3, 4, &test_mask(0xC2_5EED_0005));
    let op = parse_generic(&proof.proof_bytes);
    for col in 0..CW {
        let nodes = published_nodes(&op, col);
        let mut rows = opening_equations(&nodes);
        // The only linear structure the AIR still hands an attacker on the
        // Poseidon columns is the padding copy inside each cycle (rows 31=30,
        // 63=62, 95=94, 127=126); on the carry it is the model above; on the
        // lift column there is none at all.
        match col {
            0..=2 => {
                for cyc in 0..c2::NUM_HASH_CYCLES {
                    let last = cyc * c2::HASH_CYCLE_LEN + c2::HASH_CYCLE_LEN - 1;
        ...
        assert!(verdict.is_none(), "col {col} of C2 is recoverable from the published bytes");
    }
}
