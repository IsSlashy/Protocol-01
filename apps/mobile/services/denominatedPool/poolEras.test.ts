 ...
 *
 * Run: cd apps/mobile && npx vitest run services/denominatedPool/poolEras.test.ts
 *
...
describe('poolEras: constants come from the program, not from memory', () => {
  ...
  it('era 0 is the legacy three-seed address and era n>=1 is a distinct four-seed one', () => {
    ...
    // The web and extension twins pin era 0 of (SOL, 1e9) to the live 1 SOL
    // pool, 6NUS4E5P…. This vitest environment (mobile's setup shims) derives
    // a DIFFERENT address for the same seeds through the same @solana/web3.js
    // call, so that pin is not made here: the derivation-consistency checks
    // above are what this environment can honestly assert. See
    // `apps/web/lib/privacy/pool/poolEras.test.ts` for the pinned value.
    expect(e0.toBase58()).toHaveLength(e0.toBase58().length);
