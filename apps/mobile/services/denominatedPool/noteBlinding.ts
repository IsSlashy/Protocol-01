 ...
 * The two files must derive byte-identical values; see the domain-separator note
 ...
 *
 * 🚨 THIS IS HALF OF A TWO-PART FIX, AND MOBILE WAS MISSING ITS HALF.
 ...
 * publishing. C7 went live on devnet on 2026-08-25 while this file did not exist
 * — so every note minted from this app was relinkable to its deposit no matter
 * what the circuit did. apps/web has had the blinding since `fc6591ee`.
 *
 ...
 * and a note deposited from the web app derives a DIFFERENT blinding on mobile,
 * which makes it unrecoverable by scan and unspendable — the funds are on chain
 * and no client can name them.
 *
 * ⛔ THE LANDMINE. Once this lands, `ShieldReceipt.depositEpoch` stops being an
 * epoch and becomes a ~63-bit SECRET. Anything that publishes it hands the
 * blinding factor straight to an observer and makes this file pointless. The
 * known channel is the `min_epoch` instruction argument, which every client must
 * keep pinned to `0n`; the on-chain handler ignores it
 * (`unshield_denominated_stark_v3.rs`: `let _ = (…, min_epoch, …)`).
 */
