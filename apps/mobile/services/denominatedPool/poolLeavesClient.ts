...
/**
 * Where the indexer lives for THIS surface (the mobile app).
 *
 * `EXPO_PUBLIC_P01_WEB_URL` (the deployed apps/web origin) wins when set; else
 * the production site. `null` under vitest unless the variable is set, so
 * unit tests never reach for a network they do not have.
 */
export function resolvePoolLeavesBaseUrl(): string | null {
  const explicit = process.env.EXPO_PUBLIC_P01_WEB_URL;
  if (!explicit && process.env.VITEST) return null;
  const origin = (explicit || 'https://protocol-01.dev').replace(/\/$/, '');
