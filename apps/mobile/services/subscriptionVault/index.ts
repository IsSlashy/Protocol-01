 ...
 * absent optional is the executing program's OWN id, the same sentinel
 * convention `buildSubscribePrivateStarkIx` already uses below.
 *
 ...
 * 7,343 CU, 1 SOL paid) while the three-account form returned 3005 on the very
 * same vault. `claimAccounts.test.ts` keeps the count tied to the Rust struct.
 *
 * Native SOL only, which is every vault the product creates. For an SPL vault use
 * `buildClaimPeriodInstruction` from `@protocol-01/merchant-sdk`: the sentinels
 * make an SPL claim fail in the handler with MissingTokenProgram, which names the
 * real problem, rather than 3005.
 */
