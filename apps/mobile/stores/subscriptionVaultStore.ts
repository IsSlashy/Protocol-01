...
interface SubscriptionVaultState {
  ...
  subscribePrivateStarkAction: (
     ...
     * commitment is a leaf in the pool tree at the supplied root.
     *
     * 🚨 publicInputs is `[leaf_u64, subtree_root_u64, depth(=12)]` SINCE
     * 2026-08-29 — this doc said `root_u64, depth(=15)` and "the service derives
     * the ix `merkle_root` from `publicInputs[1]`", and both stopped being true
     * when C3 was cut to depth 12 for its blinding region. The pool root now
     * travels in `walk` below.
     */
    c3ProofData: { proofBytes: Uint8Array; publicInputs: bigint[]; proofSize: number },
    /**
     * [C3-D12] The pool root from the caller's own tree walk, plus the Merkle
     * levels ABOVE the depth-12 circuit for the handler to fold on chain.
     * ⛔ REQUIRED — `merkleRoot` is NOT `c3ProofData.publicInputs[1]`.
     */
