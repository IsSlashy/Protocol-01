/**
 * Message Store for P-01 Mobile E2E Encrypted Messaging
 *
 * Handles:
 * - Conversations and messages storage
 * - Message encryption/decryption
 * - Persistence to AsyncStorage (encrypted at rest)
 * - Read/unread status tracking
 *
 * Uses zustand with AsyncStorage for React Native compatibility.
 */
...
interface MessageStoreState {
  ...
  // Actions
  initializeEncryption: (walletSecretKey: Uint8Array) => Promise<void>;
...
export const useMessageStore = create<MessageStoreState>()(
  persist(
    (set, get) => ({
      ...
      initializeEncryption: async (walletSecretKey: Uint8Array) => {
        set({ isLoading: true });

        try {
          // Derive encryption keypair from wallet seed
          const encryptionKeys = await deriveEncryptionKeysFromSeed(walletSecretKey);

          // Derive storage key for at-rest encryption
          const storageKey = deriveStorageKey(walletSecretKey);
