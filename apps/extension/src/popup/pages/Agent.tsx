...
const QUICK_ACTIONS = [
  ...
  { label: 'Market Summary', icon: TrendingUp, color: 'text-p01-cyan' },
    ...
    <div className="flex flex-col h-full bg-p01-void">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-p01-border shrink-0">
        <div className="flex items-center gap-2">
          <h1 className="text-p01-text font-display font-bold tracking-wide">P-01 Agent</h1>
            ...
            <div className="p-4 space-y-3">
              ...
              {settingsProvider !== 'ollama' && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-p01-surface rounded-lg px-3 py-2 text-sm text-p01-text placeholder-p01-chrome/40 outline-none border border-p01-border focus:border-p01-cyan/50"
                  />
                </div>
              )}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={isTesting}
                  className="flex-1 py-2 bg-p01-surface text-p01-text text-xs font-medium rounded-lg hover:bg-p01-elevated transition-colors flex items-center justify-center gap-1.5"
                >
                ...
                <div className="space-y-1">
                  {conversations.map(conv => (
                    ...
                    >
                      ...
                      <span className="text-xs text-p01-text truncate flex-1">{conv.title}</span>
                      <button
        ...
        {!hasMessages ? (
          ...
          <div className="flex flex-col items-center pt-8 px-4">
            ...
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-p01-cyan to-p01-cyan-dim flex items-center justify-center mb-4">
              <Sparkles className="w-8 h-8 text-p01-text" />
            </div>
            <h2 className="text-p01-text font-display font-bold text-lg mb-1">P-01 Agent</h2>
            ...
            {(solPrice || fearGreed) && (
              <div className="flex items-center gap-3 px-4 py-2 bg-p01-surface rounded-full mb-6">
                {solPrice && <span className="text-xs text-p01-text">SOL ${solPrice.toFixed(2)}</span>}
            ...
            <div className="w-full grid grid-cols-2 gap-2">
              {QUICK_ACTIONS.map(action => (
                ...
                >
                  ...
                  <span className="text-xs text-p01-text font-medium">{action.label}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="p-3 border-t border-p01-border shrink-0">
        <div className="flex items-center gap-2 bg-p01-surface rounded-xl px-3 py-2">
          <input
            ...
            aria-label="Message"
            className="flex-1 bg-transparent text-sm text-p01-text placeholder-p01-chrome/40 outline-none min-w-0"
            disabled={isLoading}
          />
          <button
            onClick={() => handleSend()}
        ...
        <div
          className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
            isUser
              ...
              : 'bg-p01-surface text-p01-text rounded-bl-md'
          }`}
        >
          {message.content || (
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            {message.suggestions.map(suggestion => (
              <button
                ...
                className="px-2.5 py-1 bg-p01-surface/50 border border-p01-border rounded-full text-xs text-p01-chrome hover:bg-p01-surface hover:text-p01-text transition-colors"
              >
                {suggestion}
              </button>
            ))}
