/**
 * Gemma 3n AI Service for Specter Extension
 *
 * Provides AI-powered intent parsing and response generation using Google's Gemma 3n model.
 * Supports multiple backends:
 * - Ollama (local server with gemma3 model)
 * - Google AI Studio API (cloud-based)
 * - WebLLM (future: in-browser inference with WebGPU)
 *
...
export interface GemmaConfig {
  backend: 'ollama' | 'google-ai' | 'webllm';
  // Ollama settings
  ollamaUrl?: string;
...
const SPECTER_SYSTEM_PROMPT = `You are Specter AI, the intelligent assistant for the Specter Protocol browser extension wallet on Solana.
...
- Create subscriptions with privacy features
- Estimate gas fees
...
- stealth_send: Private/stealth transfer using derived addresses
- check_balance: Check wallet balance
- price_lookup: Token price inquiry
- create_stream: Create recurring payment stream
...
- Specter supports Solana addresses (32-44 character base58 strings)`;

const STORAGE_KEY = 'specter_gemma_config';
...
export async function testGemmaConnection(config?: GemmaConfig): Promise<{
  ...
  try {
    ...
    return { success: false, error: 'WebLLM backend not yet implemented' };
  } catch (error: any) {
    return {
      success: false,
...
export async function processWithGemma(
  ...
  try {
    if (cfg.backend === 'ollama') {
      ...
      throw new Error('WebLLM backend not yet implemented');
    }
...
function fallbackIntentParsing(userMessage: string, aiResponse: string): GemmaIntentResponse {
  ...
  if (lower.match(/^(hi|hello|hey|bonjour|salut)/)) {
    return {
      ...
      confidence: 0.9,
      entities: {},
      response: aiResponse || "Hello! I'm Specter AI. How can I help you with your wallet today?",
      suggestions: [
        { text: 'Check my balance', action: 'check_balance' },
  ...
  return {
    ...
    confidence: 0.5,
    entities: {},
    response: aiResponse || "I'm here to help you manage your Specter wallet. You can ask me to send payments, check your balance, or create payment streams.",
    suggestions: [
      { text: 'Check my balance', action: 'check_balance' },
