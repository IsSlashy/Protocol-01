...
import { getConnection as getConnectionForNetwork } from './wallet';
import { useWalletStore } from '../store/wallet';

/** Extension shim: mobile's getConnection() takes no args; the extension's is
 * network-scoped. Resolve the current network from the wallet store so the
 * ported relay client keeps its zero-arg call sites. */
function getConnection(): Connection {
  return getConnectionForNetwork(useWalletStore.getState().network);
}
...
export async function submitRelayJob(
  ...
  const { deriveEphemeralForRelay, addPendingRelay, jobIdToHex } = await import('./relayEphemeralRecovery');
...
export async function relayTransaction(
  ...
  const { removePendingRelay, markPendingRelayErrored, jobIdToHex } =
    await import('./relayEphemeralRecovery');
