 ...
 *
 * Verbatim copy of apps/web/lib/privacy/pool/poolLeavesClient.ts except for
 * `resolvePoolLeavesBaseUrl`, which is per surface. Change them together.
 */
...
/**
 * Where the indexer lives for THIS surface (the browser extension).
 *
 * - `VITE_P01_WEB_URL` (the deployed apps/web origin) wins when set;
 * - else `VITE_PAIR_API_BASE`, which already names that origin for /api/pair;
 * - else the production site.
 * `null` under vitest unless `VITE_P01_WEB_URL` is set, so unit tests never
 * reach for a network they do not have. The route answers with
 * `Access-Control-Allow-Origin: *`, so no host_permissions entry is needed.
 */
export function resolvePoolLeavesBaseUrl(): string | null {
  const env = import.meta.env;
  const explicit = env.VITE_P01_WEB_URL;
  if (!explicit && env.VITEST) return null;
  const origin = (explicit || env.VITE_PAIR_API_BASE || 'https://protocol-01.dev').replace(/\/$/, '');
