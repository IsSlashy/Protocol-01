...
// jsdom here -- not for some unlucky seed, for all of them -- so the fee-escrow
// derivation inside the instruction builder cannot run. `denominatedPool.test.ts`
// carries the same override through `environmentMatchGlobs` in vitest.config.ts;
// this states it in the file so it travels with the file.
/**
 * The EXTENSION's v4 spend instruction — pinned independently of the web twin.
 *
...
describe('extension: unshield_denominated_stark_v4 — the wire', () => {
...
describe('extension: unshield_denominated_stark_v4 — the property', () => {
...
describe('extension: recipientHashLimbs', () => {
