/**
 * Device pairing for the EXTENSION (bidirectional extension ⇄ mobile).
 *
 ...
 * paste + FORCED 6-digit SAS entry) is the authenticated out-of-band channel. The
 * mnemonic is the ONLY payload and NEVER appears in cleartext in any QR /
 * clipboard / log / relay — it travels encrypted under a hardened hybrid box
 * (X25519 + ML-KEM-768 → HKDF-SHA256 → XSalsa20-Poly1305). See
 * docs/pairing-ledger-spec.md §1A and §3 (Stage S2).
 *
 * ── SINGLE SOURCE OF TRUTH ────────────────────────────────────────────────────
 * The wire format / SAS / box below is BYTE-IDENTICAL to the SDK pairing module
 * `packages/privacy-sdk/src/pairing/{box,protocol}.ts` (Stage S1) — same prefixes,
 * domain strings, transcript ordering and field layout. The SDK box was itself
 * lifted FROM the extension's hardened `noteCrypto.ts`. It is ported here (rather
 * than imported) because the extension does NOT depend on `@protocol-01/privacy-sdk`
 * and that package ships only from an unbuilt `dist/` with no `./pairing` subpath
 * export — importing it would couple the extension typecheck/build to an SDK build
 * step. Mobile (Stage S3) consumes the SAME algorithm so a phone and this extension
 * interoperate. If you change EITHER side, change all three to keep them identical.
 *
 * Wire frames:
 ...
 * cannot swap EITHER frame without changing the displayed digits. On the paste
 * path the SAS is FORCED (the user must TYPE the 6 digits), not a one-tap confirm.
 ...
 * strings are immutable; we forbid clipboard for the raw phrase in the UI).
 */
...
// ─── prefixes / versions / domains (must match the SDK + mobile) ────────────────
...
// ─── base64 helpers (popup has btoa/atob) ───────────────────────────────────────
...
// ════════════════════════════════════════════════════════════════════════════════
//  EXTENSION ADAPTER  —  receiver / sender flows on top of the protocol above
// ════════════════════════════════════════════════════════════════════════════════
...
export interface ReceiverSession {
  ...
  bundle: PairingReceiveBundle;
  /**
   * Compute the 6-digit SAS for a scanned/pasted QR#2 blob, WITHOUT decrypting.
   * The receiver must show this and only proceed after the user confirms it
   * matches the sender's screen (paste path: typed entry is FORCED by the UI).
 ...
 * session closure. The keys never touch chrome.storage — they live for the
 * lifetime of the ceremony and are `fill(0)`-zeroized on `consumeQr2` success or
 * `cancel()`.
 */
...
/**
 * SENDER step 1: parse + validate a scanned/pasted QR#1 from the receiver.
...
export interface SenderQr2 {
   ...
   * from the scanned/pasted QR#2 and the two are compared out-of-band.
   */
  sas: string;
}
 ...
 *
 * Reads the ACTIVE wallet's mnemonic by decrypting `encryptedSeedPhrase` with the
 * cached SESSION password (set at unlock), encrypts it to the receiver's bundle,
 * and returns the QR#2 blob + the SAS the sender must show.
 *
 * GUARDS:
 *  - Refuses if the active wallet is HARDWARE (no mnemonic to send — Finding #3).
 *    Hardware wallets are flagged by walletKind on the store; pairing only ships
 *    seed-backed wallets. (Today the store has no hardware kind yet, so this is a
 *    forward-compatible guard keyed on the absence of `encryptedSeedPhrase`.)
 *  - Refuses if the wallet is locked (no session password cached).
 *
 * SECRECY: the decrypted mnemonic is held only as a local string for the duration
 * of `encodePairingPayload` and is NEVER logged. The encoded payload byte buffer is
 * `fill(0)`-zeroized after encryption.
 */
export async function senderBuildQr2(parsedQr1: PairingQr1): Promise<SenderQr2> {
  const state = useWalletStore.getState();

  // Hardware guard (forward-compatible): a wallet with no encrypted seed phrase has
  // no mnemonic to pair out. This also covers any future kind:'hardware' wallet.
  const walletKind = (state as unknown as { walletKind?: string }).walletKind;
  if (walletKind === 'hardware') {
    throw new Error('This is a hardware wallet — it has no seed phrase to send. Pairing is unavailable.');
  }
  if (!state.isInitialized || !state.encryptedSeedPhrase) {
    throw new Error('No seed-backed wallet to pair from on this device.');
  }

  const password = getSessionPassword();
  if (!password) {
    throw new Error('Wallet is locked. Unlock your wallet, then try pairing again.');
  }

  // Decrypt the active mnemonic with the session password. NEVER logged.
  const mnemonic = await decrypt(state.encryptedSeedPhrase, password);
  if (!validateMnemonic(mnemonic)) {
...
/**
 * RECEIVER convenience wrapper: given the active receiver session and the
 * scanned/pasted QR#2, return the imported `{ mnemonic, walletKind }`. Thin alias
 * over `session.consumeQr2` so callers have a symmetric sender/receiver API.
 */
