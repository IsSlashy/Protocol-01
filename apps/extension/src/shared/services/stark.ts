...
// ⚠️ TWIN OF apps/web/lib/privacy/pool/stark.ts FROM THIS LINE DOWN. The two
// files are byte-identical below the header on purpose (2026-09-06): the
// upload, the polling and the single-transaction consume are one design, and
// a resize count or a phase gate that drifts between them is a buffer too
// small or a proof marked verified on phase 1 alone. Edit both, or neither.
...
/**
 * Close a stale buffer, initialise a fresh one and grow it to the proof size,
 * then upload the bytes. Everything up to and excluding verification, shared
 * by the two-phase submit and the single-transaction consume below so the two
 * can never disagree on the resize count.
 */
...
export async function submitAndVerifyStarkProof(
  ...
  const authority = signer.publicKey;
  const proofBuffer = await prepareProofBuffer(proof, signer, connection, onProgress);
...
export async function submitAndConsumeStarkProof(
  ...
  onProgress?: (step: string) => void,
): Promise<{ proofBuffer: PublicKey; authority: PublicKey; txSignature: string; transactions: number }> {
  const authority = signer.publicKey;
  const proofBuffer = await prepareProofBuffer(proof, signer, connection, onProgress);
  ...
  const verify2 = needsPhase2(proof.circuitId)
    ...
    : null;
  const close = buildCloseProofBufferIx(proofBuffer, authority);
  ...
  const consumeAlone = async (): Promise<string> => {
    const tx = new Transaction().add(...budget(consumer.computeUnits), ...consumer.instructions, close);
    fireBeforeSend();
    return audible(`Verified; now ${consumer.label} and closing the buffer...`, onProgress, () =>
      send(tx),
    );
  };
  ...
  try {
    ...
    if (!fitsInPacket(combined, authority)) {
      ...
      return { proofBuffer, authority, txSignature: r.txSignature, transactions: n + r.transactions };
    }
    ...
    try {
      ...
      closeOnFailure = false;
      return { proofBuffer, authority, txSignature, transactions: n + 1 };
    } catch (e) {
