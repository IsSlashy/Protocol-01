...
// Real @solana/web3.js signing and PDA derivation, like denominatedPool.test.ts:
// under jsdom the polyfilled Buffer and the native one disagree inside
// findProgramAddressSync ("Unable to find a viable program address nonce").
/**
 ...
 *
 * Same fake-RPC method as the web stark.test.ts: transactions are really signed and
...
describe('this stark.ts is the web twin, byte for byte below the header', () => {
  it('matches apps/web/lib/privacy/pool/stark.ts from the first import down', () => {
    const web = readFileSync(join(__dirname, '../../../../web/lib/privacy/pool/stark.ts'), 'utf8');
    const ext = readFileSync(join(__dirname, 'stark.ts'), 'utf8');
    const marker = '\nimport {\n  ComputeBudgetProgram,';
    expect(web.indexOf(marker)).toBeGreaterThan(0);
    expect(ext.indexOf(marker)).toBeGreaterThan(0);
    // A resize count or a phase gate that drifts between the two surfaces is a
    // buffer too small or a proof marked verified on phase 1 alone. Edit both.
    expect(ext.slice(ext.indexOf(marker))).toBe(web.slice(web.indexOf(marker)));
  });
});
