    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-2 text-xs text-p01-text-muted">
          {selectedSize
            ? `${selectedSize.totalNotes} note${selectedSize.totalNotes === 1 ? "" : "s"} in this pool. Amounts snap to a denomination, so the amount you move is not distinctive. See the privacy note below for what withdrawal still reveals.`
      ...
      {/* Cost, not privacy. Kept out of the disclosure box below so nothing
          reads as a fifth thing the pool hides. Plain sentence in front, the
      ...
      <details className="text-xs text-p01-text-muted">
        <summary className="cursor-pointer marker:text-p01-text-dim">
          Shielding {denomination} SOL needs about {shieldCost} SOL free for a few minutes. Most of
          it is a refundable deposit that comes back at the end.
        </summary>
        <p className="mt-1.5 pl-4 text-p01-text-dim">
          The exact breakdown: the {denomination} SOL denomination, a 0.3% protocol fee, and ~1 SOL
          of proof-buffer rent that is returned when the buffer closes. Withdrawal charges 0.5%,
          and moving the payout off its one-time address afterwards costs one more transaction fee
          (0.000005 SOL).
        </p>
      </details>
      ...
      {/* Both flows report through the same worker `step` string; only one runs
          at a time, so each bar keys off its own running flag and renders null
          otherwise. The bar receives the step verbatim and does the mapping. */}
      <FlowProgress
      ...
      {result && !shielding && (
        <div className="space-y-2">
          ...
          <div className="card p-4">
            ...
            <p className="mt-1 text-xs text-p01-text-muted">
              It counts toward the balance above and can be withdrawn from this device.
            </p>
