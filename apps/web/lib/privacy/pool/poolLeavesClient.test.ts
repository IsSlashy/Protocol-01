...
describe('fetchPoolLeavesByIndex routing', () => {
  ...
  it('resolves no base URL under vitest unless the env var names one', () => {
    expect(resolvePoolLeavesBaseUrl()).toBeNull();
    vi.stubEnv('NEXT_PUBLIC_P01_POOL_LEAVES_URL', 'https://idx.example/api/pool-leaves/');
    try {
      expect(resolvePoolLeavesBaseUrl()).toBe('https://idx.example/api/pool-leaves');
    } finally {
      vi.unstubAllEnvs();
    }
  });
