/**
 * Goldilocks-Poseidon (pure TS port for the extension client).
 *
 * Verbatim twin of `apps/mobile/services/zk/goldilocks-poseidon.ts`.
 * If you edit one, edit both — the mobile vitest parity vectors
 * (goldilocks-poseidon.test.ts) pin the constants and permutation output
 * against the Rust reference, so any drift breaks on-chain verification.
 *
 * Reference: `stark/src/poseidon/mod.rs` + `stark/src/poseidon/constants.rs`.
 *
