...
class InProcessStarkWorker {
  ...
  constructor() {
    ...
    this.shim = {
      ...
      //
      // THIS FILE IS SAFE TODAY ONLY BY ACCIDENT OF SCOPE: shield proves C6,
      // which draws no randomness at all, so an impoverished `self` is
      // invisible here. Point this harness at a masked circuit (C7 draws a
