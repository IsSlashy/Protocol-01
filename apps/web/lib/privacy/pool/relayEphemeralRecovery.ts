...
// Web shim for the extension's chrome.storage.local (only used by the Step-2
// relay/transfer path; the Step-1 shield/unshield flow never reaches it).
// localStorage in a page context; in-memory fallback in a Worker/SSR context.
const memStore = new Map<string, string>();
const webStore = {
  get(k: string): string | null {
    try {
      if (typeof localStorage !== 'undefined') return localStorage.getItem(k);
    } catch { /* worker/SSR — fall through */ }
...
const SecureStore = {
  getItemAsync: async (k: string): Promise<string | null> => webStore.get(k),
  setItemAsync: async (k: string, v: string): Promise<void> => webStore.set(k, v),
};
const Crypto = {
