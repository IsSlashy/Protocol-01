...
| A5 | ~~Ouvertures quotient et FRI hors couverture du masque~~ **FERMÉ sur C1/C3/C6/C7 le 2026-08-31.** La colonne de lift rend **384 valeurs de quotient engagées sur 384** affines en un élément de masque, donc exactement uniformes ; la colonne randomizer couvre FRI et le terminal. ~~⛔ C5 reste OUVERT : ni lift ni randomizer~~ **FERMÉ sur C5 le 2026-09-11** : colonne de lift et colonne randomizer ajoutées (largeur 7 → 9), X2 (feuilles de quotient exactement uniformes, 8 circuits) et X5 (valeurs non ouvertes uniformes sachant tout le publié, 8 circuits) passent sur C5 ; le comptage canal B de la nouvelle forme est en cours de re-mesure (`full_wire_ledger`) | **F** sur les 8 | `compact::zk_hiding` (`quotient_leaves_are_exactly_uniform_on_every_circuit`), `full_wire_ledger.rs` |
...
| ?2 | ~~C3 et C6 : jamais attaqués~~ **FERMÉ 2026-08-31 / 2026-09-11.** Attaqués par les mêmes harnais que C1 (contrôle positif puis masque) ; sous-déterminés sur canal A, uniformité X1/X2/X5 mesurée | **F** | `air_aware_recovery_c3.rs`, `_c6.rs`, `zk_hiding` |
