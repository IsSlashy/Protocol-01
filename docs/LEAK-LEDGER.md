...
|---|---|---|---|
| A1 | **C1/C3/C6 n'ont aucun masque** : la preuve est fonction déterministe du témoin. Les 4 entrées privées de C1 (dont `secret`) sont récupérées des octets publiés | O | `air_aware_recovery_c1.rs` (épingle la *récupération*) |
...
| A3 | **Jointure inter-circuits** : un secret récupéré via C0/C1 rend le masque C7 sans objet pour la même identité (`spend.rs:920,:1052`) | O | rien |
| A4 | **Système publié surdéterminé** : ~1 600 felts publiés contre ~1 308 inconnues. L'argument de comptage (`spend.rs:1420-1449`) ne couvre que 900 d'entre eux | O | rien |
| A5 | Ouvertures **quotient** (176 pos + 176 miroir + 8 OOD) et **FRI** (308 + 32 finaux) : équations dans les mêmes inconnues, hors couverture du masque | O | rien |
| A6 | **Pas de sel FRI, feuilles Merkle non salées**, 2 024 hachages frères par preuve (`compact.rs:7239,:53`) | O | rien |
| A7 | **Colonne 9, lignes 96..383** = `hash2(secret, cycle)` : dérivé du secret, invariant d'une preuve à l'autre (`spend.rs:1152-1157`) | O | rien |
| A8 | **Le masque est fourni par l'appelant**, seule sa *longueur* est vérifiée (`compact.rs:9557-9562`) ; `draw_spend_mask` n'existe que sous `feature="wasm"` (`lib.rs:106,:357`) | O | rien |
