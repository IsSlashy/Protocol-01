...
<ul class="none">
  ...
  <li><strong>Deposit to withdraw linkage is possible on every spend but one.</strong> The subscription spend and the transfer both republish the commitment their deposit already published, so matching the two is trivial for an observer. The crowd of 47 unspent notes is a ceiling; the effective size today is one. C7 closes it for the <em>withdrawal</em> only &mdash; deployed 25 August, one real spend landed, no commitment on the wire &mdash; and there is no C7 subscription. No client routes to the C7 withdrawal yet either: all three still call the v3 path.</li>
