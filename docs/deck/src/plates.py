...
<ul class="none">
  ...
  <li><strong>Deposit to withdraw linkage is possible on four of the five spends.</strong> The v3 withdrawal, the subscription, the transfer and the split all republish the commitment their deposit already published &mdash; at instruction byte 80, 160, 80 and 80 respectively &mdash; so matching the two is trivial for an observer. The crowd of 47 unspent notes is a ceiling; the effective size today is one. C7 closes it for the <em>withdrawal</em> only &mdash; deployed 25 August, one real spend landed, no commitment on the wire &mdash; and there is no C7 subscription. No client routes to the C7 withdrawal yet either: all three still call the v3 path.</li>
