...
**Proves it:** `expected_wire_size` pin test (`compact.rs:1574-1599`) — the target is ~132 KB at ffps 32, versus 258,958 B for C1+C3 today, i.e. **the merge roughly halves upload bytes and eliminates one whole proof-buffer rent**; `circuit_7_periodic_coeffs_match_verifier_constants` comparing the **full** arrays, not the spot values the existing C1-C6 parity tests use; and `spend_proof_satisfies_deep_ali_end_to_end` cloned from `:3058`, which reconstructs the verifier's DEEP-ALI identity in-crate and so catches an alpha/periodic/quotient mismatch without needing the verifier crate at all.
