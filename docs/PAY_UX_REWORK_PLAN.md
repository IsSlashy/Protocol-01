...
## 7. Tranché par le fondateur (2026-08-04)

1. **Le style visuel actuel reste** (sombre, cyan, monospace). Ce qui change est **l'agencement**, pas la
   palette.
2. **Le mobile est la référence** à égaler pour les abonnements.
3. **Le jargon doit être simplifié.** C'est une exigence explicite, pas une préférence : le vocabulaire du
   circuit sort du premier plan.
4. **La barre de progression s'applique à TOUS les parcours**, shield, retrait, envoi, abonnement, et
   pas seulement aux plus longs.
5. **Une animation de confirmation** suit tout envoi réussi.

⛔ Rappel qui survit à toute simplification de vocabulaire : **on n'allège jamais une phrase au point de
la rendre fausse.** L'ensemble d'anonymat vaut 1, et aucun mot plus simple ne doit suggérer le contraire.
